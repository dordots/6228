...
export default function GearForm({ gear, soldiers, onSubmit, onCancel, existingGear = [], currentUser }) {
  ...
  const isWithSoldier = gear && (gear.armory_status || 'with_soldier') === 'with_soldier';
  const isInDeposit = gear && gear.armory_status === 'in_deposit';
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <p className="text-sm text-slate-600">
              {depositAction === 'deposit' 
                ? 'This will transfer the gear to deposit and unassign it from the current soldier.'
                : 'This will release the gear from deposit and set its status to "with soldier".'}
            </p>
          </div>
          <DialogFooter>
